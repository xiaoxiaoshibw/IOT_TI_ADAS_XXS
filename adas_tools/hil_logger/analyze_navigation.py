...
def main() -> int:
    ...
    if args.markdown:
        ...
        args.markdown.write_text(
            ...
            f"- 车道横向偏移 P95 / max：{shown('lane_lateral_offset_abs_p95_m', ' m')} / {shown('lane_lateral_offset_abs_max_m', ' m')}\n"
            f"- 车道航向误差 max：{shown('lane_heading_error_abs_max_rad', ' rad')}\n"
            f"- 转角速率 P95 / max：{shown('steering_rate_p95_rad_s', ' rad/s')} / {shown('steering_rate_max_abs_rad_s', ' rad/s')}\n"
            f"- 自车平均速度：{shown('ego_speed_mean_mps', ' m/s')}\n"
            f"- 加速度 P95：{shown('accel_p95_abs_mps2', ' m/s²')}\n"
            f"- 急动度 P95：{shown('jerk_p95_abs_mps3', ' m/s³')}\n"
            f"- Lead ID 切换：{shown('lead_id_switches_count')}\n"
            f"- Lead gap/speed 均值：{shown('lead_gap_mean_m', ' m')} / {shown('lead_speed_mean_mps', ' m/s')}\n"
            f"- 油门/制动同时动作：{shown('throttle_brake_simultaneous_count')}\n"
            f"- 车道无效跃迁：{shown('lane_invalid_transitions')}\n"
            f"- AEB 触发次数：{shown('aeb_trigger_count')}\n"
            f"- 安全 MRM 时间占比：{shown('safety_state_time_in_mrm_fraction')}\n"

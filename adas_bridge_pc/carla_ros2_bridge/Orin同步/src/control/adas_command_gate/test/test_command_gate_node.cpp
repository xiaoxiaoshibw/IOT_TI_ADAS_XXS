...
class CommandGateNodeShellTest : public ::testing::Test {
 protected:
...
TEST_F(CommandGateNodeShellTest, ConfigureActivateAndCleanupResources) {
  auto node = std::make_shared<adas::control::CommandGateNode>(rclcpp::NodeOptions());
  ...
  EXPECT_FALSE(node->get_publishers_info_by_topic("/adas/control/gate/control_cmd").empty());

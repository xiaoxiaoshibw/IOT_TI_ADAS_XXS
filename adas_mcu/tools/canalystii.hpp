...
class Device {
 public:
  ...
  // 非阻塞收取当前通道待收报文（返回条数）
  int receive(int channel, std::vector<RxFrame>& out) {
    ...
    int got = read_ep(kCommandEp[channel] | 0x80, resp, sizeof(resp), 200);
    ...
    int n = read_ep(kMessageEp[channel] | 0x80, data.data(), rx_size, 200);
    if (n <= 0) return 0;
